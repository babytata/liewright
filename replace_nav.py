...
files_to_check = [f for f in os.listdir(directory) if f.endswith('.php')]

old_button_prev = '<button class="scroll-btn prev" aria-label="Previous"><</button>'
old_button_next = '<button class="scroll-btn next" aria-label="Next">></button>'

# Regular expressions to match the old JavaScript logic

# For index.php
regex_index_js = re.compile(
    r"// --- Nav Carousel Logic ---.*?(?=// --- Hero Grid Logic ---)", 
    re.DOTALL
)

# For other files
regex_other_js = re.compile(
    r"const carousel = document\.querySelector\('\.nav-carousel'\);[\s\S]*?prevBtn\.addEventListener\('click', \(\) => scroll\(-1\)\);\s*nextBtn\.addEventListener\('click', \(\) => scroll\(1\)\);",
    re.DOTALL
)
...
for filename in files_to_check:
    ...
    # Replace buttons
    content = content.replace(old_button_prev, '')
    content = content.replace(old_button_next, '')
    
    # Replace JS
    if filename == 'index.php':
        content = regex_index_js.sub(new_js, content)
    else:
        content = regex_other_js.sub(new_js, content)
